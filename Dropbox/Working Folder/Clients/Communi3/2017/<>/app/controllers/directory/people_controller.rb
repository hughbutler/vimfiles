class Directory::PeopleController < Directory::AuthController
    ...
    def index
        ...
        if params[:q]
            query = params[:q]
            @people = current_community.people.where("last_name ~* ? OR first_name ~* ?", query, query)
            @people.where!("gender = ? OR is_clergy = TRUE", params[:gender]).order("last_name ASC, first_name ASC")
