...
class Weekend::Nomination < ApplicationRecord
  ...
  scope :by_community,  -> (community_id) { where('people.community_id = ?', community_id).joins(:person) }
  scope :approved,      -> (status = true) { where('is_accepted = ?', status) }
