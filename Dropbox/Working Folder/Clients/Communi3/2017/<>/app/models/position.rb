...
class Position < ActiveRecord::Base
  ...
  # Scopes
  scope :community,     -> (community_id) { where('community_id = ? and deleted = ?', community_id, false).order('title') }
  ...
  default_scope { order('category_1 asc, category_2 asc, category_3 asc, title asc') }
